...
const validateUserID = (req, res, next) => {
  knex('quotes').where('id', req.params.id).then(([data]) => {
    if (!data) {
...
const buildPatchReq = (req, res, next) => {
  ...
  const allowedPatchKeys = [quote, author]
...
router.get('/', (req, res, next) => {
  knex('quotes').then(data => res.status(200).json(data)).catch(err => next(err))
})

/* GET single emergencies/steps record */
router.get('/:id', validateUserID, (req, res, next) => {
  knex('quotes').where('id', req.params.id).then(([data]) => res.status(200).json(data)).catch(err => next(err))
})
...
router.patch('/:id', validateUserID, buildPatchReq, (req, res, next) => {
  ...
  knex('quotes').where('id', req.params.id).first().update(patchReq).returning('*').then(([data]) => {
...
router.delete('/:id', validateUserID, (req, res, next) => {
  knex('quotes').where('id', req.params.id).first().del().returning('*').then(([data]) => {
